...
namespace Verx.Autentication.Service.Api.Middleware;
...
public class ExceptionMiddleware(RequestDelegate next)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (Exception ex) when (ex is DomainException exception)
        {
            await HandlerDomainExceptionAsync(context, exception);
        }
        catch (Exception)
        {
    ...
    /// <summary>
    /// Handles domain exceptions by returning a structured response with error details.
    /// </summary>
    /// <param name="context"></param>
    /// <param name="ex"></param>
    /// <returns></returns>
    private static Task HandlerDomainExceptionAsync(HttpContext context, DomainException ex)
    {
        ...
        response.Errors = ex.Erros.Select(i => new ValidationFailure(i.Code.Contains("password", StringComparison.InvariantCultureIgnoreCase) ? "password" : "", i.Description));
